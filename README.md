...
```bash
  npm i --save-dev @applaud/html-webpack-plugin
```

```bash
  yarn add --dev @applaud/html-webpack-plugin
```
...
The `@applaud/html-webpack-plugin` provides [hooks](https://github.com/jantimon/html-webpack-plugin#events) to extend it to your needs. There are already some really powerful plugins which can be integrated with zero configuration
 ...
 * [webpack-concat-plugin](https://github.com/hxlniada/webpack-concat-plugin) for concat and uglify files that needn't to be webpack bundles(for legacy files) and inject to @applaud/html-webpack-plugin.
...
**webpack.config.js**
```js
const HtmlWebpackPlugin = require('@applaud/html-webpack-plugin')
...
If you have plugins that make use of it, `@applaud/html-webpack-plugin` should be ordered first before any of the integrated plugins.

<h2 align="center">Options</h2>

You can pass a hash of configuration options to `@applaud/html-webpack-plugin`.
